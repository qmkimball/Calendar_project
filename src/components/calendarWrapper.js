...
export default function calendarWrapper(props) {
    const renderCalendarBoxes = () => {
        ...
        for (let i = 1; i <= props.month.start_day; i++) {
            ...
            calendarBoxesArray.push(<CalendarBox key={`P-${i} `} date={date} overflow />);
        }

        for (let i = 1; i <= props.month.days_in_month; i++) {
            calendarBoxesArray.push(<CalendarBox key={`{props.month.id}-${i}`} date={i} month={props.month} />);
        }
    ...
    return(
        <div className="calendar-wrapper" style={{border: "3px solid lightpurple"}} >
